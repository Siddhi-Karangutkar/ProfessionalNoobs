...
import { Switch } from "@/components/ui/switch";  // Importing a toggle switch component
...
const DashboardSidebar = ({ userType }: DashboardSidebarProps) => {
  ...
  const [settingsActive, setSettingsActive] = useState(false); // State for settings activation
  ...
  const vendorLinks = [
    ...
    { name: "Settings", path: "/dashboard/vendor/settings", icon: <Settings className="h-5 w-5" />, settings: true },
  ];
  ...
  const sponsorLinks = [
    ...
    { name: "Settings", path: "/dashboard/sponsor/settings", icon: <Settings className="h-5 w-5" />, settings: true },
  ];
    ...
    <aside className="w-64 border-r bg-white h-screen sticky top-0 py-6 flex flex-col">
          ...
          {links.map((link) => (
            <li key={link.path}>
              <Link
                ...
                onClick={() => link.settings && setSettingsActive(!settingsActive)} // Activate settings when clicked
              >
                {link.icon}
                <span>{link.name}</span>
              </Link>
      ...
      <div className="mt-4 px-3">
        <div className="p-4 rounded-lg bg-gray-50 flex items-center justify-between">
          <h4 className="font-medium text-sm">Settings Active</h4>
          <Switch
