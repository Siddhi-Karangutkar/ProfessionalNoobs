      ...
      <div className="w-1/3 border-r p-4 space-y-4">
        ...
        <div className="space-y-2">
          {filteredMessages.map((message) => (
            <div
              key={message.id}
              onClick={() => setSelectedConversation(message.sender_id === user?.id ? message.receiver_id : message.sender_id)}
              className="p-3 border rounded-lg hover:bg-gray-50 cursor-pointer"
            >
              <div className="flex items-center justify-between">
                <div>
                  <h4 className={`font-medium ${!message.read ? 'text-brand-purple' : 'text-gray-900'}`}>
                    {message.sender.business_name}
                  </h4>
                  <p className="text-sm text-gray-600 line-clamp-1">{message.content}</p>
                </div>
                {!message.read && <Badge variant="purple">New</Badge>}
              </div>
            </div>
          ))}
        </div>
